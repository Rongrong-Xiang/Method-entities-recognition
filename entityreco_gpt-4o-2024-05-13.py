...
# ----------------------------
# 3. 定义调用 GPT API 的函数（实体识别），并加入少量学习示例
# ----------------------------

# 定义少量学习示例（few-shot examples），你可以根据实际情况进行调整
few_shot_examples = '''
Example 1:
...
Entities: [{"id": 603, "label": "Dataset", "start_offset": 358, "end_offset": 364}, {"id": 604, "label": "Indicator", "start_offset": 457, "end_offset": 472}, {"id": 605, "label": "Data_Analysis", "start_offset": 496, "end_offset": 524}]
...
Entities: [{"id": 67, "label": "Dataset", "start_offset": 438, "end_offset": 485}, {"id": 68, "label": "Data_Analysis", "start_offset": 506, "end_offset": 529}, {"id": 69, "label": "Data_Analysis", "start_offset": 570, "end_offset": 598}]
'''

def clean_json_output(output):
    """
    尝试修正 GPT 输出的 JSON 字符串：
      - 去除 Markdown 代码块标记
      - 补全不完整的 JSON（例如末尾缺 "]"）
    """
...
test_input_file = "/workspace/Innovation_method_202503/input_data/2_sample_nonmanual_labeled.jsonl"
results = []
...
for sample in data_samples:
    ...
    print(f"PaperID {paper_id} processed. GPT Entities: {filtered_entities}")
    print("-" * 50)
    sleep(3)

# 保存测试集输出结果为 JSON Lines 文件
output_test_file = "/workspace/Innovation_method_202503/output_data/gpt4o_method_nonmanual_output.jsonl"
...
# # 模型输出文件，包含预测实体结果
# output_file = "/workspace/Innovation_method_202503/output_data/gpt_method_test_output.jsonl"
